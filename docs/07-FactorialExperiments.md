...
<img src="07-FactorialExperiments_files/figure-html/resin-intxn-1.png" alt="4x2 interaction plot" width="480" />
...
+ $a$: number of levels of factor "A" (for the resin data, we'll set "acid" as this factor, so $a = 2$.)
...
<!-- Two possible interaction plots for the above means are: -->
<!-- ```{r rat-intxn, echo = FALSE, fig.width = 8, fig.cap = "Interaction plots of the rat weight-gain data"} -->
...
<!-- plot(c(-0.5,1.5),c(500,700),type="n",xlab="type of fat",ylab="Weight gain",xaxt="n") -->
...
<!-- legend("topright", col = c("blue", "red"), leg = c("male", "female"), lty = "solid") -->
...
<!-- plot(c(-0.5,1.5),c(500,700),type="n",xlab="Sex",ylab="Weight gain",xaxt="n") -->
<!-- axis(1,at=0:1,lab=c("Male","Female")) -->
<!-- lines(c(0,1)-.02,c(mean(male.fresh),mean(female.fresh)),col="blue",lwd=2) -->
<!-- lines(c(0,1)+.02,c(mean(male.rancid),mean(female.rancid)),col="red",lwd=2) -->
<!-- points(c(0,1)-.02,c(mean(male.fresh),mean(female.fresh)),pch="F",lwd=2) -->
<!-- points(c(0,1)+.02,c(mean(male.rancid),mean(female.rancid)),pch="R",lwd=2) -->
<!-- legend("topright", col = c("blue", "red"), leg = c("fresh", "rancid"), lty = "solid") -->

<!-- #legend(0,35,leg=c("Control","Treated"),col=c("blue","red"),pch=c("C","T")) -->
<!-- add.bars<-function(x,y,se,...) segments(x,y-se,x,y+se,...) -->
<!-- add.bars(0-.02,mean(male.fresh),sqrt(var(male.fresh))/sqrt(3),col="blue",lwd=2) -->
<!-- add.bars(1-.02,mean(female.fresh),sqrt(var(female.fresh))/sqrt(3),col="blue",lwd=2) -->
<!-- add.bars(0+.02,mean(male.rancid),sqrt(var(male.rancid))/sqrt(3),col="red",lwd=2) -->
<!-- add.bars(1+.02,mean(female.rancid),sqrt(var(female.rancid))/sqrt(3),col="red",lwd=2) -->
<!-- ``` -->
...
In the usual way, testing each of these contrasts entails computing a sum-of-squares associated with each, standardizing that sum-of-squares by its associated df to generate a mean-square, and then comparing the mean-square for the contrast to the MSE to generate an $F$-statistic.  
...
<img src="07-FactorialExperiments_files/figure-html/unnamed-chunk-5-1.png" width="480" style="display: block; margin: auto;" />
...
## Unreplicated factorial designs {#unreplicated-designs}
