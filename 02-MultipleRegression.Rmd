...
# ===== Left panel: nicotine versus tar, with PC1 axis =====

plot(cig$tar, cig$nicotine,
     xlab = "Tar",
     ylab = "Nicotine",
     pch = 19)
...
# Direction of PC1 in the original tar/nicotine units
dx <- w["tar"]      * pca$scale["tar"]
dy <- w["nicotine"] * pca$scale["nicotine"]

# PC1 line in original units
slope <- dy / dx

intercept <- pca$center["nicotine"] -
             slope * pca$center["tar"]

abline(a = intercept, b = slope, lwd = 2)

# Choose integer PC1 values covering the observed scores
pcvals <- floor(min(PC1)):ceiling(max(PC1))

# Coordinates corresponding to these PC1 values
xtick <- pca$center["tar"] +
         pcvals * w["tar"] * pca$scale["tar"]

ytick <- pca$center["nicotine"] +
         pcvals * w["nicotine"] * pca$scale["nicotine"]

# Draw short tick marks perpendicular to PC1.
#
# The conversion through par("usr") accounts for the very
# different numerical scales of tar and nicotine.
usr <- par("usr")
pin <- par("pin")

# Direction of PC1 in physical plotting units
dx.in <- dx / diff(usr[1:2]) * pin[1]
dy.in <- dy / diff(usr[3:4]) * pin[2]

# Perpendicular unit vector in physical plotting units
d <- sqrt(dx.in^2 + dy.in^2)
px.in <- -dy.in / d
py.in <-  dx.in / d

# Half-length of each tick, in inches
tick.length <- 0.06

# Convert perpendicular direction back to data units
px <- px.in * tick.length * diff(usr[1:2]) / pin[1]
py <- py.in * tick.length * diff(usr[3:4]) / pin[2]

segments(xtick - px, ytick - py,
         xtick + px, ytick + py)

# Label PC1 values beside the tick marks
text(xtick + 2.5 * px,
     ytick + 2.5 * py,
     labels = pcvals,
     cex = 0.8)

# ===== Right panel: CO versus PC1, with regression line =====
