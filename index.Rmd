...
```{r fig.cap = "BAC vs. beers consumed.", echo = FALSE }
...
```{r fig.cap = "SLR fit of BAC vs. beers consumed."}
with(beer, plot(BAC ~ Beers, xlab = "beers consumed"))
abline(fm1)
```
...
with(beer, plot(BAC ~ Beers, xlab = "beers consumed"))
abline(fm1)
segments(x0 = 9, y0 = 0.19, y1 = fitted(fm1)[3], col = "red")
```
...
```{r echo = FALSE}
fly <- read.table("data/fruitfly.txt", head = T, stringsAsFactors = T)
fly_ancova <- lm(life ~ trt + thorax, data = fly)

plot(resid(fly_ancova) ~ fitted(fly_ancova), xlab = "Fitted values", ylab = "Residuals")
...
```{r qqexamples, echo = FALSE, fig.width = 5.5, fig.height = 9}
set.seed(1)
par(mfrow = c(3, 2))

my.function <- function(x) dgamma(x, shape = 1.5, rate = 1)
curve(my.function, from = 0, to = 5, xlab = "right skew (common)", ylab = "", main = "Underlying distribution\nof residuals")
ttt <- rgamma(100, shape = 1.5, rate = 1)
qqnorm(ttt, main = "Example of\ncorresponding Q-Q plot")
qqline(ttt)

my.function <- function(x) dgamma(-x, shape = 1.5, rate = 1)
curve(my.function, from = -5, to = 0, xlab = "left skew (rare)", ylab = "")
ttt <- -1 * rgamma(100, shape = 1.5, rate = 1)
qqnorm(ttt, main = "")
qqline(ttt)

my.function <- function(x) dt(x, df = 1)
curve(my.function, from = -3, to = 3, xlab = "fat tails (rare)", ylab = "")
ttt <- rt(100, df = 1)
qqnorm(ttt, main = "")
qqline(ttt)
```
...
```
```{r echo = FALSE}
plot(BAC ~ Beers, data = beer,
     xlab = "No. of beers consumed",
     ylab = "BAC",
     ylim = c(0, 0.2))
...
new.data <- data.frame(Beers = seq(from = min(beer$Beers),to = max(beer$Beers), length = 100))
ttt <- predict(fm1, interval = "confidence", newdata = new.data)
lines(new.data$Beers, ttt[,2], lty = "dashed")
lines(new.data$Beers, ttt[,3], lty = "dashed")
ttt <- predict(fm1, interval = "prediction", newdata = new.data)
lines(new.data$Beers, ttt[,2], lty = "dotted")
lines(new.data$Beers, ttt[,3], lty = "dotted")
```
...
```{r power curves, echo = FALSE}
nVals <- 5:40
rsqVals <- c(0.3, 0.6)
powerVals <- matrix(NA, nrow = length(nVals), ncol = length(rsqVals))

for (i in seq(along = nVals)) {
  for (j in seq(along = rsqVals)) {
  powerVals[i, j] <- pwrss::pwrss.f.reg(r2 = rsqVals[j], k = 1, n = nVals[i], alpha = 0.05, verbose = F)$power
  }
}

plot(powerVals[, 1] ~ nVals, xlab = "sample size, n", ylab = "Power", ylim = c(0, 1), type = "l", bty = "n", xaxs = "i", yaxs = "i")
lines(powerVals[,2] ~ nVals, col = "red")
legend("bottomright", leg = c("R2 = 0.6", "R2 = 0.3"), pch = 16, col = c("red", "black"), bty = "n")
```

The plot below shows the power as a function of $R^2$ for several sample sizes.

```{r more power curves, echo = FALSE}
nVals <- c(5, 20, 50)
rsqVals <- seq(from = 0, to = 0.99, by = 0.01)
powerVals <- matrix(NA, nrow = length(nVals), ncol = length(rsqVals))
...
plot(powerVals[1, ] ~ rsqVals, xlab = expression(R^2), ylab = "Power", ylim = c(0, 1), xlim = c(0, 1), type = "l", bty = "n", xaxs = "i", yaxs = "i")
lines(powerVals[2, ] ~ rsqVals, col = "blue")
lines(powerVals[3, ] ~ rsqVals, col = "red")
legend("bottomright", leg = c("n = 50", "n = 20", "n = 5"), pch = 16, col = c("red", "blue", "black"), bty = "n")
```
