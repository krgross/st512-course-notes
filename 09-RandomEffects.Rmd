...
sources <- c("Plant", "Leaf(Plant)", "Error", "Total")

df <- c("3 (= 4 - 1)", "8 (= 12 - 1- 3)", "12 (= 24 - 1- 3 - 8)", 23)

var_comp_anova_table <- data.frame(cbind(sources, df))

colnames(var_comp_anova_table) = c("source", "df")

knitr::kable(format="html",
             var_comp_anova_table,
             escape = FALSE)
```
...
rose_anova_table <- data.frame(cbind(sources = c("Treatment", "Vase (EU error)","Stems (subsample error)", "Total"),
                                      df = c(5, "24 (= 30 - 1- 5)", "60 \ (= 90 - 1 - 5 - 24)", 89)))
