...
```{r resin-intxn, echo = FALSE, fig.alt = "4x2 interaction plot", fig.cap = "Interaction plot for the pine resin data"}
...
sources <- c("Factor A marginal means", "Factor B marginal means", "A*B interaction")

df <- c("$a-1$", "$b-1$", "$(a-1)(b-1)$")

two_factor_anova_table <- data.frame(cbind(sources, df))

colnames(two_factor_anova_table) = c("ANOVA contrast", "df")
