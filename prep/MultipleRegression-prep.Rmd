...
```{r}
beer <- read.table("data/beer2.csv", head = T, stringsAsFactors = T)
head(beer)
```
...
\begin{description}
\item[acetic] the natural log of acetic acid concentration
\item[h2s] the natural log of hydrogen sulfide concentration
\item[lactic] the concentration of lactic acid
\end{description}
...
\begin{center}
	\includegraphics*[width = 5in]{figures/cheese-pairs}
\end{center} 

Let's entertain a model that uses all three predictors.  In R, 
...
{\em Example:} Form a 99\% CI for $\beta_2 $, the partial regression coefficient associated with weight.  First we find the critical value in R:
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
...
{\em Example:} Test $H_0$: $\beta_2 =0$ vs. H$_{a}$: $\beta_2 \ne 0$ at the $\alpha$=5\% significance level.
...
> predict(fm1, newdata = new.data, interval = "prediction")

            fit        lwr        upr
[1,] 0.05808664 0.03480226 0.08137103

> predict(fm1, newdata = new.data, interval = "confidence")
...
\textbf{Categorical variables} are variables that cannot be sensibly placed on a number line.  Examples include gender, ethnicity, or brand of manufacture.  We use \textbf{indicator variables} as devices to include categorical variables as predictors in regression models.  

{\em Constructing indicator variables.} Choose one level of the categorical variable as the reference level.  Sometimes, the scientific context of a problem will suggest that one level should serve as the reference level.  Oftentimes, though, the choice of the reference level is arbitrary.  In all cases, the choice of the reference level will have no effect on the ensuing analysis.  For every other level, create a separate indicator variable that is equal to 1 for that level, and is equal to 0 for all levels.  Thus, to include a categorical variable with $c$ levels, we need $c - 1$ indicator variables.

*Example.*  The BAC data also contain information about gender.  Let $x_3$ be an indicator variable for gender, where  
\[
x_3 =\left\{\begin{array}{cc} {1} & {{\rm male}} \\ {0} & {{\rm female}} \end{array}\right. 
\] 

Here is a look at some of the data with the indicator variable $x_3$ included:
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
     BAC weight gender beers male.id
1  0.100    132 female     5       0
2  0.030    128 female     2       0
3  0.190    110 female     9       0
4  0.120    192   male     8       1
...
16 0.050    175   male     4       1
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}
A regression model that includes beers consumed, weight, and gender as predictors is
\[
y=\beta_0 +\beta_1 x_1 +\beta_2 x_2 + \beta_3 x_3 +\epsilon 
\] 

In R, we can code this model as 
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> fm3 <- lm(BAC ~ beers + weight + male.id, data = beer)
> summary(fm3)
...
male.id     -3.240e-03  6.286e-03  -0.515 0.615584    
...
For males, the above model is:
\begin{eqnarray*}
y & = & \beta_0 +\beta_1 x_1 +\beta_2 x_2 +\beta_3 +\epsilon  \\ 
  & = & \left(\beta_0 +\beta_3 \right)+\beta_1 x_1 +\beta_2 x_2 +\epsilon
\end{eqnarray*}
For females, the model is:
\[
y=\beta_0 +\beta_1 x_1 +\beta_2 x_2 +\epsilon 
\] 
Thus, the partial regression coefficient $\beta_3 $ quantifies the difference in BAC for men vs. women after controlling for the effects of the number of beers consumed and weight.
...
R note:  If R recognizes that a variable is categorical, it will construct indicators on its own.  However, it takes a little detective work to find out what the reference level is.  For example, in the BAC data, there is a separate variable called 'gender' that takes the values 'male' and 'female'.  If we tried the model

\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> fm5 <- lm(BAC ~ beers + weight + gender, data = beer)
> summary(fm5)
...
The model fit is clearly the same as the model that used our manually created indicator 'male.id'.  However, to determine how to interpret the sign of the estimated coefficient for 'gendermale', we need to know what level R chose as the reference.  To find out, we need to use the contrasts function:
...
This tells us that the indicator variable "gendermale" takes the value 0 for females and 1 for males.  Thus, in this case R chose 'female' as the reference gender.  (R will typically assign the reference to whatever level of the categorical variable it encounters first.  Here, the first observation in the data set is for a female, so female became the reference level.)

{\em Example:}  Here is an example of a data set that includes a categorical predictor variable with more than two levels.   The data set \textit{cars} contains the following observations for 140 vehicles, all manufactured in 1999:

\begin{itemize}
  \item \textit{displacement}: a measure of volume
  \item \textit{hp}: horsepower
  \item \textit{drive}: a categorical variable with 3 levels: "4-wheel", "front", and "rear"
  \item \textit{weight}: weight of the vehicle 
  \item \textit{mpgcity}: fuel efficiency (miles per gallon) in the city
  \item \textit{mpghw}: fuel efficiency (miles per gallon) on the highway
\end{itemize}
...
displacement         hp            drive        weight         mpghw          mpgcity     
Min.   : 61.0   Min.   : 55.0   4-wheel:35   Min.   :2125   Min.   :19.60   Min.   :10.90  
1st Qu.:134.3   1st Qu.:140.0   front  :49   1st Qu.:3375   1st Qu.:26.68   1st Qu.:18.27  
Median :181.0   Median :189.0   rear   :56   Median :3750   Median :31.95   Median :20.70  
Mean   :191.5   Mean   :188.3                Mean   :3868   Mean   :32.05   Mean   :21.44  
3rd Qu.:244.0   3rd Qu.:220.0                3rd Qu.:4250   3rd Qu.:35.92   3rd Qu.:24.30  
Max.   :364.0   Max.   :436.0                Max.   :6000   Max.   :60.10   Max.   :46.60
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}

To include "drive" in a regression model, we need two indicator variables:
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
  displacement  hp   drive weight mpghw mpgcity front.id rear.id
1          122 132   front   2875  40.5    26.1        1       0
2          150 125    rear   3375  27.5    19.9        0       1
3          150 120    rear   3625  33.8    24.1        0       1
4          150 120 4-wheel   3625  25.3    19.8        0       0
5          239 175    rear   4500  21.6    16.9        0       1
...
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}
...
{\em Example 1:} You are studying the effect of temperature on weight gain in fish, and measure temperature in both degrees Fahrenheit and Centigrade.  (Recall that one can convert between Fahrenheit and Centigrade by the equation F = (9/5) C + 32.)  The design matrix is
\[
...
{\em Example 2:}  With the beer data, suppose we created indicator variables for both males and females, and entertained the model:
\[
...
\] 
In both of these examples, it is impossible to fit the regression model because the matrix  $\left(\X'\X\right)^{-1} $does not exist.  Here is an example of what happens if we tried to fit the temperature model to a made-up data set:
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
...
...
16 0.050    175   male     4 -0.8125   3.4375
...
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> fm4 <- lm(BAC ~ beers + weight + male.id + beers:male.id, data = beer)
> summary(fm4)
...
male.id        1.518e-02  1.251e-02   1.213 0.250523    
beers:male.id -3.946e-03  2.368e-03  -1.666 0.123824    
...
Note: When we included the beer*male.id interaction in the model, we also included the indicator male.id alone.  To not have done so would have constrained the model to have the same intercept for men and women, despite the fact that the slopes with respect to the number of beers consumed could have differed.  Such a constraint makes little sense.  This is an example of a general rule of thumb, which is:  If a model includes an interaction between two predictors, it must include each of the individual predictors as well.
...
The machinery of MLR can be used to model a non-linear relationship between a predictor and a response.  This is called {\em polynomial regression}.  A $k^{th}$ order polynomial regression model is
\[
...
{\em Example:} In the cars data, the relationship between highway mpg and vehicle weight is clearly non-linear:
\begin{center}
	\includegraphics*[width=2.5in]{figures/cars}
\end{center}
...
% Exercise: Use this model to predict the highway mpg of a 4000-lb vehicle.
% Solution: 29.9 mpg
...
weight      -4.175e-02  2.033e-02  -2.054   0.0420 *  
I(weight^2)  6.504e-06  4.984e-06   1.305   0.1941    
I(weight^3) -3.715e-10  3.966e-10  -0.937   0.3506    
...
\begin{center}
	\includegraphics*[height=2.5in]{figures/cars-quad}
\end{center}
...
\begin{center}
	\includegraphics*[height=2.5in]{figures/fish}
\end{center}
...
y=\beta_0 +\beta_1 x_1 +\beta_2 x_2 +\beta_3 x_3 +\epsilon 
\] 
where $y$ is the log of tissue mercury content, $x_1$ is age (in years), $x_2$ is an indicator for Adger Lake, and $x_3$ is an indicator for Bennett's Millpond.  That is, 
\begin{displaymath}
x_2 = \begin{cases} 1 & \texttt{lake=Adger} \\ 0 & \texttt{otherwise} \end{cases}
\end{displaymath}
and
\begin{displaymath}
x_3 = \begin{cases} 1 & \texttt{lake=Bennett's Millpond} \\ 0 & \texttt{otherwise.} \end{cases}
\end{displaymath}
...
\item{Step 1}.  Fit the "full" model.  The full model is the one provides the context for the hypothesis test.  Record the SSE and the df for error.
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> full.model <- lm(log(tissue.hg) ~ age + adger.id + bennett.id, data = fish)
> summary(full.model)

Coefficients:
            Estimate Std. Error t value Pr(>|t|)    
(Intercept) -2.06723    0.16351 -12.643 1.07e-10 ***
age          0.14309    0.01967   7.276 6.66e-07 ***
adger.id     0.26105    0.10817   2.413   0.0261 *  
bennett.id   0.33212    0.13689   2.426   0.0254 *  
...
> sum(resid(full.model)^2)
[1] 0.8252884

\end{verbatim}
\renewcommand{\baselinestretch}{1.5}

\item{Step 2.} Fit a "reduced" model.  The reduced model is found by taking the full model and imposing the null hypothesis on it.  In the fish example, the reduced model is $y=\beta_0 +\beta_1 x_1 +\epsilon$.   Record the SSE and the df for error.
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> reduced.model <- lm(log(tissue.hg) ~ age, data = fish)
...
> sum(resid(reduced.model)^2)
[1] 1.172507
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}

\item{Step 3.}  Calculate an $F$-statistic.  The formula for the $F$-statistic is:
\[
...
Model 1: log(tissue.hg) ~ age
Model 2: log(tissue.hg) ~ age + adger.id + bennett.id
...
\begin{enumerate}
\item Models that include an interaction between predictors should also include the predictors individually.
\item Models that include polynomial powers of a predictor should include all lower-order terms of that predictor.
\end{enumerate}
...
\begin{enumerate}
	\item  Adjusted $R^2$.  Adjusted $R^2$ is a penalized version of $R^2$ that imposes a penalty for each additional parameter added to the model.  The (rather opaque) formula for adjusted $R^2$ is
	\[
	{\rm Adj-}R^2 =1-\frac{n-1}{n-\left(k+1\right)} \left(1-R^2 \right)
	\] 
	The model with the largest Adj-$R^2 $ is considered best.
	
	\item PRESS statistic (PRESS = PRedicted Sum of Squares)
	
	\begin{itemize}
		\item Step 1: Remove the first data point.
		
		\item Step 2: Fit the model to the remaining data.
		
		\item Step 3. Use the model from step 2 to predict the removed data point, $\hat{y}_1^* $.
		
		\item Step 4.  Add the first data point back to the data set.  
		
		\item Step 5.  Repeat steps 1-4 for each data point in turn.
	\end{itemize}
...
There are three different types of sequential methods, based on the direction in which the model is built.  In {\em forward selection}, we begin with the simplest possible model (namely, $y = \beta_0 + \epsilon$), and grow the model by adding predictors to it.  In {\em backwards elimination}, we start with the most expansive possible model, and shrink it by removing unnecessary predictors.  In {\em stepwise selection}, we start with the simplest possible model (namely, $y = \beta_0 + \epsilon$), and then merge forwards and backwards steps, either growing and shrinking the model until converging on one that cannot be improved.  Stepwise selection is used more often than the other two variants.  Each of the three procedures could possibly lead to a different best model.

Here is the algorithm for forward selection:

\begin{enumerate}
	\item Initiation step: Start with the model $y=\beta_0 +\epsilon$.  This is the initial "working" model.
	\item Iteration step: Fit a set of candidate models, each of which differs from the working model by the inclusion of a single additional model term.  Be aware that the set of candidate models must abide our rules of thumb about hierarchy.  (That is, we wouldn't consider a model like $y = \beta_0 + \beta_1 x_1 x_2 + \epsilon$.)
	\item Ask: Do any of the candidate models improve upon the working model?  
	\begin{enumerate}
		\item If the answer to this question is "yes", then choose the model that improves upon the working model the most.  Making this model the working model, and begin a new iteration (return to step 2).
		\item (Termination step): If the answer to this question is "no", then stop.  The current working model is the final model. 
	\end{enumerate}
\end{enumerate}

The algorithm for backwards elimination is similar:
\begin{enumerate}
	\item Initiation step: Start with the most expansive possible model.  Usually, this will be the model with all possible predictors, and potentially with all possible first- (and conceivably second-)order interactions.  Note that we can consider a quadratic term to be an interaction of a predictor with itself in this context.  This is the initial "working" model.
	\item Iteration step: Fit a set of candidate models, each of which differs from the working model by the elimination of a single model term.  Again, be aware that the set of candidate models must abide our rules of thumb about hierarchy, so (for example) we wouldn't consider a model that removes $x_1$ if the interaction $x_1 x_2$ is still in the model.
	\item (Same as forward selection.)  Ask: Do any of the candidate models improve upon the working model?  
	\begin{enumerate}
		\item If the answer to this question is "yes", then choose the model that improves upon the working model the most.  Making this model the working model, and begin a new iteration (return to step 2).
		\item (Termination step): If the answer to this question is "no", then stop.  The current working model is the final model. 
	\end{enumerate}
...
> # the second argument of `step' defines the largest possible model
...
+ H2S     1    602.48 1400.49  119.30
+ Lactic  1    578.76 1424.21  119.81
+ Acetic  1    480.19 1522.78  121.81
<none>                2002.97  128.04
...
+ Lactic  1    115.94 1284.54  118.71
<none>                1400.49  119.30
+ Acetic  1     73.60 1326.88  119.68
- H2S     1    602.48 2002.97  128.04
...
Df Sum of Sq     RSS     AIC
<none>                1284.54  118.71
- Lactic  1    115.94 1400.49  119.30
- H2S     1    139.67 1424.21  119.81
+ Acetic  1     27.19 1257.36  120.07
...
\begin{itemize}
\item Step 1. Examine univariate (ST511) summaries of the data (summary statistics, boxplots, etc.).  Identify unusual values or possible problems.  (Don't take it on faith that your data are all correct!)

\item Step 2.  Examine scatterplots with all variables.  Find variables that are closely correlated with the response and with each other.

\item Step 3.  Candidate model selection: Identify a model that includes relevant variables.  Use automated selection procedures if you wish.

\item Step 4:  Assumption checking: Check (standardized) residuals.  Determine if polynomial terms or transformations may be needed.

\item Step 5: Examine collinearity diagnostics.  Inspect VIFs and pairwise correlations between variables.  Decide if some variables may be removed or added.

\item Step 6: Revision.  Add or remove terms based on steps 4-5.  Return to step 3.

\item Step 7: Prediction and testing: Consider validating the model with a sample that was not included in building the model.
\end{itemize}
...
{\em Ex. Puromycin.}  This example is taken directly from the text \textit{Nonlinear regression analysis and its applications}, by D.M. Bates and D.G. Watts (1988, Wiley).  The data themselves are from Treloar (1974, MS Thesis, Univ of Toronto), who studied the relationship between the velocity of an enzymatic reaction (the response, measured in counts / minute$^2$) vs. the concentration of a particular substrate (the predictor, measured in parts per million).  The experiment was conducted in the presence of the enzyme Puromycin.  The data are shown below.
\begin{center}
	\includegraphics*[width=2.5in]{figures/puromycin-raw}
\end{center} 

It is hypothesized that these data can be described by the Michaelis-Menten model for enzyme kinetics.  The Michaelis-Menten model is:
\[
...
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
> fm1 <- nls(velocity ~ theta1 * conc / (theta2 + conc), data = enzyme, 
...
Achieved convergence tolerance: 6.101e-06
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}
...
\begin{center}
	\includegraphics*[height=2.5in]{figures/puromycin-fit}
\end{center} 
...
\begin{center}
	\includegraphics*[height=2.5in]{figures/puromycin-poly}
\end{center} 

## Leverage, influential points, and standardized residuals}

Recall that in SLR, a data point can have undue influence on the regression model if the value of the predictor, $x$, is for away from $\bar{x}$.  The same notion applies in MLR: data points can be unduly influential if their combination of predictors lies far away from the "center of mass" of the other predictors.  However, with multiple predictors, it is harder to detect influential points visually.  The \textit{leverage} of a data point is a measure of its distance from the center of mass of the predictors, and hence its influence.  The formula for calculating leverages is complicated, so we'll use a computer to calculate them.  Leverages are usually denoted with the letter $h$, so the leverage for the $i$th data point is $h_i$.  If we looked at the equation for a leverage, however, we would discover that leverages are strictly functions of the predictors, and do not depend on th response.
...
\renewcommand{\baselinestretch}{1}
\begin{verbatim}
     BAC weight gender beers   lev
1  0.100    132 female     5 0.112
2  0.030    128 female     2 0.195
3  0.190    110 female     9 0.518
4  0.120    192   male     8 0.203
5  0.040    172   male     3 0.111
6  0.095    250 female     7 0.259
7  0.070    125 female     3 0.147
...
16 0.050    175   male     4 0.073
\end{verbatim}
\renewcommand{\baselinestretch}{1.5}
Not surprisingly, the point with the greatest leverage is the 110-lb person who drank 9 beers.

What qualifies as a large leverage?  Quinn \& Keough's text, p. 95, gives one possible rule of thumb:  "{\dots}a useful criterion is that any observation with a leverage value greater than $2\left(k+1\right)/n$ should be checked (Hoaglin \& Welsch 1978)."  
...
\textbf{2.14 More on sums-of-squares decompositions }

\textbf{}

It is tempting to try to partition the SS(Regression) into components that can be assigned to each of the several predictors.  Unfortunately, (except in special cases) there is no "ideal" decomposition.  Two different types of decompositions are possible, each of which has their own pros and cons.  The SAS software system has labeled these decomposition "Type I" and "Type III" SS.  (This terminology has nothing to do with Type I and Type II errors.)  These labels have not been universally accepted or adopted, but they are sufficiently well embedded in the agricultural literature that you should be familiar with their use.  An explanation of Type I and Type III SS in regression is provided here for your reference, although this material will not be a point of emphasis in ST512 (i.e., you don't need to know it for the exams).



Consider the following 4 models from the BAC example:



Model 0: BAC = intercept + error

Model 1: BAC = intercept + beers + error

Model 2: BAC = intercept + weight + error

Model 3: BAC = intercept + beers + weight + error



In R,

> fm0<-lm(BAC~1,data=beer) \# note that "1" indicates an intercept only

> fm1<-lm(BAC~beers,data=beer)

> fm2<-lm(BAC~weight,data=beer)

> fm3<-lm(BAC~beers+weight,data=beer)



\begin{tabular}{|p{0.9in}|p{1.0in}|p{0.9in}|p{0.9in}|} \hline 
Model & SS(Regression) & SS(Residual) & SS(Total) \\ \hline 
0 (intercept only) & 0 & 0.0292 & 0.0292 \\ \hline 
1 (beers) & 0.0234 & 0.0058 & 0.0292 \\ \hline 
2 (weight) & 0.0007 & 0.0285 & 0.0292 \\ \hline 
3 (beers + weight) & 0.0278 & 0.0014 & 0.0292 \\ \hline 
\end{tabular}



Type I SS, or \underbar{sequential} SS, measure the additional variability explained by adding a new predictor to a regression model.  Sequential SS can be produced in R using the command anova.
...
beers      1 0.0233753 0.0233753 215.688 1.791e-09 ***

weight     1 0.0044408 0.0044408  40.976 2.339e-05 ***

Residuals 13 0.0014089 0.0001084    



Because "beers" appears first on the right-hand side of model 3, the sequential (Type I) SS associated with "beers" is the reduction in SS(Residual) from adding "beers" to a model with an intercept only: 0.0292 -- 0.0058 = 0.0234.  Because "weight" appears second on the right-hand side of model 3, the sequential (Type I) SS associated with "weight" is the reduction in SS(Residual) from adding "weight" to a model that already contains "beers": 0.0058 -- 0.0014 = 0.0044.



The advantage to Type I (sequential) SS is that they are additive, i.e., the type I SS associated with beers + the type I SS associated with weight = SS(Regression) for the full model.  The disadvantage to Type I SS is that they depend on the order in which variables are placed in the model.  That is, if our full model were BAC = intercept + weight + beers + error, then the type I SS decomposition is

> fm4<-lm(BAC~weight+beers,data=beer)
...
weight     1 0.0007018 0.0007018   6.4756   0.02444 *  

beers      1 0.0271143 0.0271143 250.1883 7.162e-10 ***

Residuals 13 0.0014089 0.0001084                       



This dependence on order is highly undesirable.



An alternative to Type I (sequential SS) is partial SS.  Even more confusingly, SAS calls partial SS "Type II" SS in some contexts and "Type III" SS in others.  The "Type III" terminology is more common.



Partial (Type III) SS are defined as the difference between SS(Regression) for the model being considered and SS(Regression) for a model that contains all the same model terms except for the term for which the Type III SS is being calculated.  R does have a function to compute Type III SS.  Instead, these need to be computed by hand.



Type III SS for "beers": 0.0278 -- 0.0007 = 0.0271

Type III SS for "weight": 0.0278 -- 0.0234 = 0.0044
