...
```{r echo = FALSE, fig.height =2.5, fig.width = 8.5}
fly <- read.table("data/fruitfly.txt", head = T, stringsAsFactors = T)

par(mfrow = c(1, 5), mar = c(3, 0, 1, 0), oma = c(2, 5, 1, 1))
trt.labels <- c("alone", "1 mated", "8 mated", "1 unmated", "8 unmated")

for (i in seq(along = levels(fly$trt))) {

  with(fly, plot(life ~ thorax, type = "n", xlab = "", ylab = "", yaxt = "n"))
  with(subset(fly, trt == levels(fly$trt)[i]), points(life ~ thorax))
  
  mtext(text = trt.labels[i], side = 3, line = 0)
...
mtext(text = "thorax length (mm)", side = 1, line = 0, outer = T)
mtext(text = "lifespan (days)", side = 2, line = 3, outer = T)
```
...
```{r echo = FALSE}
fm1 <- lm(life ~ trt + thorax, data = fly)
...
for (i in seq(along = levels(fly$trt))) {
  ...
  with(fly, plot(log(life) ~ thorax, type = "n", xlab = "", ylab = "", yaxt = "n"))
  with(subset(fly, trt == levels(fly$trt)[i]), points(log(life) ~ thorax))
...
mtext(text = "thorax length (mm)", side = 1, line = 0, outer = T)
mtext(text = "log(lifespan)", side = 2, line = 3, outer = T)
```
...
```{r echo = FALSE}
fm2 <- lm(log(life) ~ trt + thorax, data = fly)
plot(resid(fm2) ~ fitted(fm2), xlab = "Fitted values", ylab = "Residuals")
abline(h = 0, lty = "dashed")
```
...
exercise <- read.table("data/exercise.txt", head = T, stringsAsFactors = T)

  par(mfrow = c(1, 3), mar = c(3, 0, 1, 0), oma = c(2, 5, 1, 1))
  
  trt.labels <- c("program 1", "program 2", "program 3")
  
  fm1 <- lm(hrate ~ -1 + program + initrate : program, data = exercise)
    
  intercepts <- coef(fm1)[1:3]
  slopes     <- coef(fm1)[4:6]
  
  for (i in seq(along = levels(exercise$program))) {
    
    with(exercise, plot(hrate ~ initrate, type = "n", xlab = "", ylab = "", yaxt = "n"))
    with(subset(exercise, program == levels(exercise$program)[i]), points(hrate ~ initrate))
  ...
  mtext(text = "initial resting heart rate (bpm)", side = 1, line = 0, outer = T)
  mtext(text = "heart rate after exercise (bpm)", side = 2, line = 3, outer = T)
```
