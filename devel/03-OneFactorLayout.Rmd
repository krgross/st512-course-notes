...
library(SASmarkdown)
# library(kableExtra) generates an error; might need to update R version to make this work

# for help using SAS in Markdown see 
...
sources <- c("Groups", "Error", "Total")

df <- c("$g-1$", "$n_T-g$", "$n_T-1$")

SS <- c("$SS_{Groups}$", "$SS_{Error}$", "$SS_{Total}$")

MS <- c("$MS_{Groups}$", "$MS_{Error}$", NA)

Fstat <- c("$MS_{Groups} / MS_{Error}$", NA, NA)

pval <- rep(NA, 3)

one_factor_anova_table <- data.frame(cbind(sources, df, SS, MS, Fstat, pval))

colnames(one_factor_anova_table) = c("source", "df", "SS", "MS", "$F$", "$p$")
                            
knitr::kable(format="html",
             one_factor_anova_table, 
             escape = FALSE) 

# eventually I want 
#knitr::kable(format="html",
#             df_accounting, 
#             escape = FALSE) %>% kable_styling()
#
# but this requires loading kableExtra, which I can't figure out how to do right now.
```
